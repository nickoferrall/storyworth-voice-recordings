  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="mx-auto w-full md:w-[741px] md:relative md:-mt-5 px-6 pt-10 pb-10">
        ...
        {error && <p className="text-red-600">There was a problem loading your calls.</p>}
        {!loading && !error && (
          <>
            {calls.length === 0 ? (
              <p className="text-slate-500 mt-4">No calls yet.</p>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {calls.map((c) => (
                    <TableRow
                      ...
                      onClick={() =>
                        (window.location.href = `/call/${encodeURIComponent(String((c as any)?.retellCallId || (c as any)?.id))}`)
                      }
                    >
                      <TableCell>
                      ...
                      <TableCell>
                        {c?.createdAt && c?.endedAt
                          ? `${Math.max(0, Math.round((new Date(c.endedAt as any).getTime() - new Date(c.createdAt as any).getTime()) / 1000))}s`
                          : '-'}
                      </TableCell>
